...
public class NodeJS {
    ...
    public String getNodeVersion() {
        ...
        try {
            ...
            safeRelease((Releasable) process);
            safeRelease((Releasable) versions);
        }
        return nodeVersion;
    }
    ...
    public static NodeJS createNodeJS(final File file) {
        ...
        try {
            File startupScript = createTemporaryScriptFile();
            try {
    ...
    public void exec(final File file) {
        ...
        try {
            ...
            safeRelease((Releasable) process);
            safeRelease((Releasable) parameters);
            safeRelease((Releasable) scriptExecution);
        }
    }
    ...
    private static File createTemporaryScriptFile() throws IOException {
        File tempFile = File.createTempFile(NodeJS.STARTUP_SCRIPT_NAME, TMP_JS_EXT);
        try (PrintWriter writer = new PrintWriter(tempFile, "UTF-8")) {
            writer.print(NodeJS.STARTUP_SCRIPT);
        }
        return tempFile;
    }
}
