...
class LibraryLoader {
    ...
    static {
        DELIMITER = System.lineSeparator(); //换行符
        SEPARATOR = FileSystems.getDefault().getSeparator(); //路径分隔符，linux上是“/”，Windows上是“\”
    }
    ...
    static void chmod(final String path) {
        if (PlatformDetector.OS.isWindows()) {
            return;
        }
        try {
            Runtime.getRuntime().exec(new String[] { "chmod", "755", path }).waitFor(); //$NON-NLS-1$
