...
inline std::string
convertStringViewToSTDString(Isolate *isolate, const v8_inspector::StringView stringView) {
...
public:
    InspectorDelegate(const function<void(std::string)> &onResponse,
                      const function<void(void)> &waitFrontendMessage) {
...
public:
    V8InspectorClientImpl(Isolate *isolate, const std::unique_ptr<v8::Platform> &platform,
        ...
        session_ = inspector_->connect(kContextGroupId, channel_.get(), v8_inspector::StringView(),
                                       inspector_->ClientTrustLevel::kFullyTrusted);
...
void throwParseException(JNIEnv *env, const Local<Context> &context, Isolate *isolate,
                         TryCatch *tryCatch);

void throwExecutionException(JNIEnv *env, const Local<Context> &context, Isolate *isolate,
                             TryCatch *tryCatch, jlong v8RuntimePtr);
...
jobject getResult(JNIEnv *env, const Local<Context> &context, jobject &v8, jlong v8RuntimePtr,
                  Handle<Value> &result, jint expectedType);
...
JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1getVersion(JNIEnv *env, jclass) {
    const char *utfString = v8::V8::GetVersion();
    return env->NewStringUTF(utfString);
}


JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1getConstructorName
...
Local<String> createV8String(JNIEnv *env, Isolate *isolate, jstring &string) {
    ...
    MaybeLocal<String> twoByteString = String::NewFromTwoByte(isolate, unicodeString,
                                                              v8::NewStringType::kNormal, length);
...
Handle<Value>
getValueWithKey(JNIEnv *env, const Local<Context> &context, Isolate *isolate, jlong &v8RuntimePtr,
                jlong &objectHandle, jstring &key) {
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<PersistentBase<Object> *>(objectHandle));
    Local<String> v8Key = createV8String(env, isolate, key);
    return object->Get(context, v8Key).ToLocalChecked();
}

void
addValueWithKey(JNIEnv *env, const Local<Context> context, Isolate *isolate, jlong &v8RuntimePtr,
                jlong &objectHandle, jstring &key, Handle<Value> value) {
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<PersistentBase<Object> *>(objectHandle));
    const uint16_t *unicodeString_key = env->GetStringChars(key, nullptr);
    int length = env->GetStringLength(key);
    Local<String> v8Key = String::NewFromTwoByte(isolate, unicodeString_key,
                                                 v8::NewStringType::kNormal,
                                                 length).ToLocalChecked();
    Maybe<bool> maybe = object->Set(context, v8Key, value);
...
//取代原jsWindowObjectAccessor方法，一分为二
static void jsWindowObjectGetter(const FunctionCallbackInfo<v8::Value> &args) {
    // 获取当前上下文的全局对象并返回
    args.GetReturnValue().Set(args.GetIsolate()->GetCurrentContext()->Global());
}

//取代原jsWindowObjectAccessor方法，一分为二
static void jsWindowObjectSetter(const v8::FunctionCallbackInfo<v8::Value> &args) {
    // 处理属性赋值
    v8::String::Utf8Value value(args.GetIsolate(), args[0]);
    std::string str(*value);
    std::cout << "Setting value: " << str << std::endl;
}
...
JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM *vm, void *) {
    ...
    // on first creation, store the JVM and a handle to deeke classes
    jvm = vm;
    v8cls = (jclass) env->NewGlobalRef((env)->FindClass("top/deeke/jni/v8/V8"));
    v8InspectorCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/inspector/V8Inspector"));
    v8InspectorDelegateCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/inspector/V8InspectorDelegate"));
    v8ObjectCls = (jclass) env->NewGlobalRef((env)->FindClass("top/deeke/jni/v8/V8Object"));
    v8ArrayCls = (jclass) env->NewGlobalRef((env)->FindClass("top/deeke/jni/v8/V8Array"));
    v8TypedArrayCls = (jclass) env->NewGlobalRef((env)->FindClass("top/deeke/jni/v8/V8TypedArray"));
    v8ArrayBufferCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8ArrayBuffer"));
    v8FunctionCls = (jclass) env->NewGlobalRef((env)->FindClass("top/deeke/jni/v8/V8Function"));
    undefinedV8ObjectCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8Object$Undefined"));
    undefinedV8ArrayCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8Array$Undefined"));
    ...
    v8ResultsUndefinedCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8ResultUndefined"));
    v8ScriptCompilationCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8ScriptCompilationException"));
    v8ScriptExecutionException = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8ScriptExecutionException"));
    v8RuntimeExceptionCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("top/deeke/jni/v8/V8RuntimeException"));
    errorCls = (jclass) env->NewGlobalRef((env)->FindClass("java/lang/Error"));
    unsupportedOperationExceptionCls = (jclass) env->NewGlobalRef(
            (env)->FindClass("java/lang/UnsupportedOperationException"));

    // Get all method IDs
    v8ArrayInitMethodID = env->GetMethodID(v8ArrayCls, "<init>", "(Ltop/deeke/jni/v8/V8;)V");
    v8TypedArrayInitMethodID = env->GetMethodID(v8TypedArrayCls, "<init>",
                                                "(Ltop/deeke/jni/v8/V8;)V");
    v8ArrayBufferInitMethodID = env->GetMethodID(v8ArrayBufferCls, "<init>",
                                                 "(Ltop/deeke/jni/v8/V8;Ljava/nio/ByteBuffer;)V");
    v8ArrayGetHandleMethodID = env->GetMethodID(v8ArrayCls, "getHandle", "()J");
    v8CallVoidMethodID = (env)->GetMethodID(v8cls, "callVoidJavaMethod",
                                            "(JLtop/deeke/jni/v8/V8Object;Ltop/deeke/jni/v8/V8Array;)V");
    ...
    throwableGetMessageMethodID = env->GetMethodID(throwableCls, "getMessage",
                                                   "()Ljava/lang/String;");
    ...
    v8CallObjectJavaMethodMethodID = (env)->GetMethodID(v8cls, "callObjectJavaMethod",
                                                        "(JLtop/deeke/jni/v8/V8Object;Ltop/deeke/jni/v8/V8Array;)Ljava/lang/Object;");
    ...
    v8ScriptCompilationInitMethodID = env->GetMethodID(v8ScriptCompilationCls, "<init>",
                                                       "(Ljava/lang/String;ILjava/lang/String;Ljava/lang/String;II)V");
    v8ScriptExecutionExceptionInitMethodID = env->GetMethodID(v8ScriptExecutionException, "<init>",
                                                              "(Ljava/lang/String;ILjava/lang/String;Ljava/lang/String;IILjava/lang/String;Ljava/lang/Throwable;)V");
    ...
    v8RuntimeExceptionInitMethodID = env->GetMethodID(v8RuntimeExceptionCls, "<init>",
                                                      "(Ljava/lang/String;)V");
    ...
    v8FunctionInitMethodID = env->GetMethodID(v8FunctionCls, "<init>", "(Ltop/deeke/jni/v8/V8;)V");
    v8ObjectInitMethodID = env->GetMethodID(v8ObjectCls, "<init>", "(Ltop/deeke/jni/v8/V8;)V");

    v8InspectorDelegateOnResponseMethodID = env->GetMethodID(v8InspectorDelegateCls, "onResponse",
                                                             "(Ljava/lang/String;)V");
    v8InspectorDelegateWaitFrontendMessageMethodID = env->GetMethodID(v8InspectorDelegateCls,
...
void V8Inspector::onResponse(const string &message) {
    JNIEnv *env;
    getJNIEnv(env);
    env->CallVoidMethod(delegate, v8InspectorDelegateOnResponseMethodID,
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1setFlags
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1getBuildID
        (JNIEnv *, jclass) {
    return 2;
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1startNodeJS
        (JNIEnv *env, jclass, jlong, jstring) {
    (env)->ThrowNew(unsupportedOperationExceptionCls, "startNodeJS Not Supported.");
}

JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1pumpMessageLoop
        (JNIEnv *env, jclass, jlong) {
    (env)->ThrowNew(unsupportedOperationExceptionCls, "pumpMessageLoop Not Supported.");
    return false;
}

JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1isRunning
        (JNIEnv *env, jclass, jlong v8RuntimePtr) {
    (env)->ThrowNew(unsupportedOperationExceptionCls, "isRunning Not Supported.");
    return false;
}

JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1isNodeCompatible
        (JNIEnv *env, jclass) {
    (env)->ThrowNew(unsupportedOperationExceptionCls, "isNodeCompatible Not Supported.");
    return false;
}

JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1createIsolate
    ...
    if (globalAlias == nullptr) {
        ...
        //取代原SetAccessor方法，一分为二
        globalObject->SetAccessorProperty(
                utfAlias,  // 属性名称
                v8::FunctionTemplate::New(runtime->isolate, jsWindowObjectGetter),  // getter 函数
                v8::FunctionTemplate::New(runtime->isolate, jsWindowObjectSetter)  // getter 函数
        );
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1createInspector
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jobject inspectorDelegateObj,
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1dispatchProtocolMessage
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1schedulePauseOnNextStatement
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1acquireLock
    ...
    if (runtime->isolate->InContext()) {
        ...
        jthrowable exception = static_cast<jthrowable>(env->NewObject(v8RuntimeExceptionCls,
                                                                      v8RuntimeExceptionInitMethodID,
                                                                      exceptionString));
        (env)->Throw(exception);
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1releaseLock
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    V8Runtime *runtime = reinterpret_cast<V8Runtime *>(v8RuntimePtr);
    if (runtime->isolate->InContext()) {
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1lowMemoryNotification
        (JNIEnv *, jobject, jlong v8RuntimePtr) {
    V8Runtime *runtime = reinterpret_cast<V8Runtime *>(v8RuntimePtr);
    runtime->isolate->LowMemoryNotification();
}

JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initEmptyContainer
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Object
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1getGlobalObject
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    return reinterpret_cast<jlong>(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->globalObject);
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1createTwin
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jlong twinObjectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,)
    Handle<Object> obj = Local<Object>::New(isolate,
                                            *reinterpret_cast<Persistent<Object> *>(objectHandle));
    reinterpret_cast<Persistent<Object> *>(twinObjectHandle)->Reset(
            reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, obj);
}

JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Int8Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Int8Array> array = Int8Array::New(arrayBuffer, static_cast<size_t>(offset),
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8UInt8Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Uint8Array> array = Uint8Array::New(arrayBuffer, static_cast<size_t>(offset),
                                              static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8UInt8ClampedArray
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Uint8ClampedArray> array = Uint8ClampedArray::New(arrayBuffer,
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Int32Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Int32Array> array = Int32Array::New(arrayBuffer, static_cast<size_t>(offset),
                                              static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8UInt32Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Uint32Array> array = Uint32Array::New(arrayBuffer, static_cast<size_t>(offset),
                                                static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8UInt16Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Uint16Array> array = Uint16Array::New(arrayBuffer, static_cast<size_t>(offset),
                                                static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Int16Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Int16Array> array = Int16Array::New(arrayBuffer, static_cast<size_t>(offset),
                                              static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Float32Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Float32Array> array = Float32Array::New(arrayBuffer, static_cast<size_t>(offset),
                                                  static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Float64Array
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong bufferHandle, jint offset, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<ArrayBuffer> arrayBuffer = Local<ArrayBuffer>::New(isolate,
                                                              *reinterpret_cast<Persistent<ArrayBuffer> *>(bufferHandle));
    Local<Float64Array> array = Float64Array::New(arrayBuffer, static_cast<size_t>(offset),
                                                  static_cast<size_t>(length));
    Persistent<Object> *container = new Persistent<Object>;
    container->Reset(reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate, array);
    return reinterpret_cast<jlong>(container);
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8ArrayBuffer__JI
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jint capacity) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    std::unique_ptr<v8::BackingStore> backing_store = v8::ArrayBuffer::NewBackingStore(isolate,
...
JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1initNewV8ArrayBuffer__JLjava_nio_ByteBuffer_2I
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1terminateExecution
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    if (v8RuntimePtr == 0) {
        return;
    }
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1releaseRuntime
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    if (v8RuntimePtr == 0) {
        return;
    }
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1contains
...
JNIEXPORT jobjectArray JNICALL Java_top_deeke_jni_v8_V8__1getKeys
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
...
ScriptOrigin *
createScriptOrigin(JNIEnv *env, Isolate *isolate, jstring &jscriptName, jint jlineNumber = 0,
                   jint jcolumnNumber = 0) {
    Local<String> scriptName = createV8String(env, isolate, jscriptName);
    return new ScriptOrigin(
            scriptName,
            jlineNumber,
            jcolumnNumber,
            false,
...
bool compileScript(const Local<Context> &context, Isolate *isolate, jstring &jscript, JNIEnv *env,
                   jstring jscriptName, jint &jlineNumber, Local<Script> &script,
                   TryCatch *tryCatch) {
...
bool runScript(const Local<Context> &context, Isolate *isolate, JNIEnv *env, Local<Script> *script,
               TryCatch *tryCatch, jlong v8RuntimePtr) {
...
bool runScript(const Local<Context> &context, Isolate *isolate, JNIEnv *env, Local<Script> *script,
               TryCatch *tryCatch, Local<Value> &result, jlong v8RuntimePtr) {
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1executeVoidScript
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jstring jjstring,
         jstring jscriptName = nullptr, jint jlineNumber = 0) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,)
    TryCatch tryCatch(isolate);
    Local<Script> script;
    if (!compileScript(context, isolate, jjstring, env, jscriptName, jlineNumber, script,
                       &tryCatch))
        return;
    runScript(context, isolate, env, &script, &tryCatch, v8RuntimePtr);
}

JNIEXPORT jdouble JNICALL Java_top_deeke_jni_v8_V8__1executeDoubleScript
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jstring jjstring,
         jstring jscriptName = nullptr, jint jlineNumber = 0) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    TryCatch tryCatch(isolate);
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1executeBooleanScript
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jstring jjstring,
         jstring jscriptName = nullptr, jint jlineNumber = 0) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false)
    TryCatch tryCatch(isolate);
...
JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1executeStringScript
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jstring jjstring,
         jstring jscriptName = nullptr, jint jlineNumber = 0) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL)
    TryCatch tryCatch(isolate);
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1executeIntegerScript
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jstring jjstring,
         jstring jscriptName = nullptr, jint jlineNumber = 0) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    TryCatch tryCatch(isolate);
...
JNIEXPORT jobject JNICALL Java_top_deeke_jni_v8_V8__1executeScript
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jint expectedType, jstring jjstring,
         jstring jscriptName = nullptr, jint jlineNumber = 0) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL)
    TryCatch tryCatch(isolate);
    Local<Script> script;
    Local<Value> result;
    if (!compileScript(context, isolate, jjstring, env, jscriptName, jlineNumber, script,
                       &tryCatch)) { return nullptr; }
    if (!runScript(context, isolate, env, &script, &tryCatch, result,
                   v8RuntimePtr)) { return nullptr; }
    return getResult(env, context, v8, v8RuntimePtr, result, expectedType);
}

bool
invokeFunction(JNIEnv *env, const Local<Context> &context, Isolate *isolate, jlong &v8RuntimePtr,
               jlong &receiverHandle, jlong &functionHandle, jlong &parameterHandle,
               Handle<Value> &result) {
    int size = 0;
    Handle<Value> *args = nullptr;
    if (parameterHandle != 0) {
        Handle<Object> parameters = Local<Object>::New(isolate,
                                                       *reinterpret_cast<Persistent<Object> *>(parameterHandle));
    ...
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(functionHandle));
    Handle<Object> receiver = Local<Object>::New(isolate,
                                                 *reinterpret_cast<Persistent<Object> *>(receiverHandle));
...
bool
invokeFunction(JNIEnv *env, const Local<Context> &context, Isolate *isolate, jlong &v8RuntimePtr,
               jlong &objectHandle, jstring &jfunctionName, jlong &parameterHandle,
               Handle<Value> &result) {
    Local<String> functionName = createV8String(env, isolate, jfunctionName);
    Handle<Object> parentObject = Local<Object>::New(isolate,
                                                     *reinterpret_cast<Persistent<Object> *>(objectHandle));
    int size = 0;
    Handle<Value> *args = nullptr;
    if (parameterHandle != 0) {
...
JNIEXPORT jobject JNICALL Java_top_deeke_jni_v8_V8__1executeFunction__JJJJ
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jlong receiverHandle, jlong functionHandle,
         jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL)
    Handle<Value> result;
    if (!invokeFunction(env, context, isolate, v8RuntimePtr, receiverHandle, functionHandle,
                        parameterHandle, result))
        return nullptr;
    return getResult(env, context, v8, v8RuntimePtr, result, top_deeke_jni_v8_V8_UNKNOWN);
}

JNIEXPORT jobject JNICALL Java_top_deeke_jni_v8_V8__1executeFunction__JIJLjava_lang_String_2J
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jint expectedType, jlong objectHandle,
         jstring jfunctionName, jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL)
    Handle<Value> result;
    if (!invokeFunction(env, context, isolate, v8RuntimePtr, objectHandle, jfunctionName,
                        parameterHandle, result))
        return nullptr;
    return getResult(env, context, v8, v8RuntimePtr, result, expectedType);
}

JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1executeIntegerFunction
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jlong objectHandle, jstring jfunctionName,
         jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<Value> result;
    if (!invokeFunction(env, context, isolate, v8RuntimePtr, objectHandle, jfunctionName,
...
JNIEXPORT jdouble JNICALL Java_top_deeke_jni_v8_V8__1executeDoubleFunction
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jlong objectHandle, jstring jfunctionName,
         jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Value> result;
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1executeBooleanFunction
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jlong objectHandle, jstring jfunctionName,
         jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Value> result;
...
JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1executeStringFunction
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jlong objectHandle, jstring jfunctionName,
         jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL)
    Handle<Value> result;
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1executeVoidFunction
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jlong objectHandle, jstring jfunctionName,
         jlong parameterHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    Handle<Value> result;
    invokeFunction(env, context, isolate, v8RuntimePtr, objectHandle, jfunctionName,
                   parameterHandle, result);
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addUndefined
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    addValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key, Undefined(isolate));
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addNull
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    addValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key, Null(isolate));
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1add__JJLjava_lang_String_2I
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key, jint value) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    addValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key,
                    Int32::New(isolate, value));
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1add__JJLjava_lang_String_2D
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1add__JJLjava_lang_String_2Ljava_lang_String_2
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1add__JJLjava_lang_String_2Z
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key,
         jboolean value) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    addValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key,
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addObject
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key,
         jlong valueHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    Handle<Value> value = Local<Object>::New(isolate,
                                             *reinterpret_cast<Persistent<Object> *>(valueHandle));
    addValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key, value);
}

JNIEXPORT jobject JNICALL Java_top_deeke_jni_v8_V8__1get
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jint expectedType, jlong objectHandle,
         jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, nullptr)
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1getInteger
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
...
JNIEXPORT jdouble JNICALL Java_top_deeke_jni_v8_V8__1getDouble
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Value> v8Value = getValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key);
    ASSERT_IS_NUMBER(v8Value);
...
JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1getString
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Value> v8Value = getValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key);
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1getBoolean
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Value> v8Value = getValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key);
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1getType__JJLjava_lang_String_2
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring key) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Value> v8Value = getValueWithKey(env, context, isolate, v8RuntimePtr, objectHandle, key);
...
bool isNumber(int type) {
    return type == top_deeke_jni_v8_V8_DOUBLE || type == top_deeke_jni_v8_V8_INTEGER;
}

bool isObject(int type) {
    return type == top_deeke_jni_v8_V8_V8_OBJECT || type == top_deeke_jni_v8_V8_V8_ARRAY;
}
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1getArrayType
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(objectHandle));
    int length = 0;
    if (array->IsTypedArray()) {
        if (array->IsFloat64Array()) {
            return top_deeke_jni_v8_V8_DOUBLE;
        } else if (array->IsFloat32Array()) {
            return top_deeke_jni_v8_V8_FLOAT_32_ARRAY;
        } else if (array->IsInt32Array()) {
            return top_deeke_jni_v8_V8_INT_32_ARRAY;
        } else if (array->IsUint32Array()) {
            return top_deeke_jni_v8_V8_UNSIGNED_INT_32_ARRAY;
        } else if (array->IsInt16Array()) {
            return top_deeke_jni_v8_V8_INT_16_ARRAY;
        } else if (array->IsUint16Array()) {
            return top_deeke_jni_v8_V8_UNSIGNED_INT_16_ARRAY;
        } else if (array->IsInt8Array()) {
            return top_deeke_jni_v8_V8_INT_8_ARRAY;
        } else if (array->IsUint8Array()) {
            return top_deeke_jni_v8_V8_UNSIGNED_INT_8_ARRAY;
        } else if (array->IsUint8ClampedArray()) {
            return top_deeke_jni_v8_V8_UNSIGNED_INT_8_CLAMPED_ARRAY;
        }
        return top_deeke_jni_v8_V8_INTEGER;
    } else {
        length = Array::Cast(*array)->Length();
    }
    int arrayType = top_deeke_jni_v8_V8_UNDEFINED;
    for (int index = 0; index < length; index++) {
        ...
        if (type < 0) {
            ...
            arrayType = top_deeke_jni_v8_V8_DOUBLE;
        } else if (isObject(arrayType, type)) {
            arrayType = top_deeke_jni_v8_V8_V8_OBJECT;
        } else {
            return top_deeke_jni_v8_V8_UNDEFINED;
        }
    }
    return arrayType;
}

JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetSize
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
    if (array->IsTypedArray()) {
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetInteger
...
int fillIntArray(JNIEnv *env, const Local<Context> context, Handle<Object> &array, int start,
                 int length, jintArray &result) {
...
int fillDoubleArray(JNIEnv *env, const Local<Context> context, Handle<Object> &array, int start,
                    int length, jdoubleArray &result) {
...
int fillByteArray(JNIEnv *env, const Local<Context> context, Handle<Object> &array, int start,
                  int length, jbyteArray &result) {
...
int
fillBooleanArray(JNIEnv *env, const Local<Context> context, Isolate *isolate, Handle<Object> &array,
                 int start, int length, jbooleanArray &result) {
...
int
fillStringArray(JNIEnv *env, const Local<Context> context, Isolate *isolate, Handle<Object> &array,
                int start, int length, jobjectArray &result) {
    for (int i = start; i < start + length; i++) {
        Handle<Value> v8Value = array->Get(context, i).ToLocalChecked();
        ASSERT_IS_STRING(v8Value);
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetIntegers__JJII_3I
...
JNIEXPORT jintArray JNICALL Java_top_deeke_jni_v8_V8__1arrayGetIntegers__JJII
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetDoubles__JJII_3D
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length,
         jdoubleArray result) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> array = Local<Object>::New(isolate,
...
JNIEXPORT jdoubleArray JNICALL Java_top_deeke_jni_v8_V8__1arrayGetDoubles__JJII
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetBooleans__JJII_3Z
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length,
...
JNIEXPORT jbyteArray JNICALL Java_top_deeke_jni_v8_V8__1arrayGetBytes__JJII
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jbooleanArray JNICALL Java_top_deeke_jni_v8_V8__1arrayGetBooleans__JJII
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetBytes__JJII_3B
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length,
         jbyteArray result) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> array = Local<Object>::New(isolate,
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1arrayGetStrings__JJII_3Ljava_lang_String_2
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length,
         jobjectArray result) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> array = Local<Object>::New(isolate,
...
JNIEXPORT jobjectArray JNICALL Java_top_deeke_jni_v8_V8__1arrayGetStrings__JJII
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1arrayGetBoolean
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint index) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jbyte JNICALL Java_top_deeke_jni_v8_V8__1arrayGetByte
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint index) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jdouble JNICALL Java_top_deeke_jni_v8_V8__1arrayGetDouble
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint index) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1arrayGetString
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint index) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT jobject JNICALL Java_top_deeke_jni_v8_V8__1arrayGet
        (JNIEnv *env, jobject v8, jlong v8RuntimePtr, jint expectedType, jlong arrayHandle,
         jint index) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, NULL);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayNullItem
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
    if (array->IsTypedArray()) {
        Local<String> string = String::NewFromUtf8(isolate,
                                                   "Cannot push to a Typed Array.").ToLocalChecked();
        v8::String::Value strValue(isolate, string);
        throwV8RuntimeException(env, &strValue);
        return;
    }
    uint32_t index = Array::Cast(*array)->Length();
    Maybe<bool> maybe = array->Set(context, index, Null(isolate));
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayUndefinedItem
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
    ...
    uint32_t index = Array::Cast(*array)->Length();
    Maybe<bool> maybe = array->Set(context, index, Undefined(isolate));
}


JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayIntItem
    ...
    uint32_t index = Array::Cast(*array)->Length();
    Maybe<bool> maybe = array->Set(context, index, v8Value);
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayDoubleItem
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayBooleanItem
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayStringItem
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1addArrayObjectItem
    ...
    uint32_t index = Array::Cast(*array)->Length();
    Local<Value> v8Value = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(valueHandle));
    Maybe<bool> maybe = array->Set(context, index, v8Value);
}
...
int getType(Handle<Value> v8Value) {
    if (v8Value.IsEmpty() || v8Value->IsUndefined()) {
        ...
        return top_deeke_jni_v8_V8_NULL;
    } else if (v8Value->IsInt32()) {
        return top_deeke_jni_v8_V8_INTEGER;
    } else if (v8Value->IsNumber()) {
        return top_deeke_jni_v8_V8_DOUBLE;
    } else if (v8Value->IsBoolean()) {
        return top_deeke_jni_v8_V8_BOOLEAN;
    } else if (v8Value->IsString()) {
        return top_deeke_jni_v8_V8_STRING;
    } else if (v8Value->IsFunction()) {
        return top_deeke_jni_v8_V8_V8_FUNCTION;
    } else if (v8Value->IsArrayBuffer()) {
        return top_deeke_jni_v8_V8_V8_ARRAY_BUFFER;
    } else if (v8Value->IsTypedArray()) {
        return top_deeke_jni_v8_V8_V8_TYPED_ARRAY;
    } else if (v8Value->IsArray()) {
        return top_deeke_jni_v8_V8_V8_ARRAY;
    } else if (v8Value->IsObject()) {
        return top_deeke_jni_v8_V8_V8_OBJECT;
    }
    return -1;
}

JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1getType__JJI
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1getType__JJ
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<Value> v8Value = Local<Value>::New(isolate,
                                              *reinterpret_cast<Persistent<Value> *>(objectHandle));
    return getType(v8Value);
}

JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1getType__JJII
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong arrayHandle, jint start, jint length) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    Handle<Object> array = Local<Object>::New(isolate,
                                              *reinterpret_cast<Persistent<Object> *>(arrayHandle));
...
jobject
createParameterArray(JNIEnv *env, const Local<Context> &context, jlong v8RuntimePtr, jobject v8,
                     int size, const FunctionCallbackInfo<Value> &args) {
    Isolate *isolate = getIsolate(env, v8RuntimePtr);
...
void voidCallback(const FunctionCallbackInfo<Value> &args) {
    ...
    jobject jreceiver = getResult(env, context, v8, md->v8RuntimePtr, receiver,
                                  top_deeke_jni_v8_V8_UNKNOWN);
    ...
    if (env->ExceptionCheck()) {
        ...
        env->ExceptionClear();
        jstring exceptionMessage = static_cast<jstring>(env->CallObjectMethod(
                reinterpret_cast<V8Runtime *>(md->v8RuntimePtr)->pendingException,
                throwableGetMessageMethodID));
        if (exceptionMessage != nullptr) {
            ...
            isolate->ThrowException(
                    String::NewFromUtf8(isolate, "Unhandled Java Exception").ToLocalChecked());
        }
    }
...
void objectCallback(const FunctionCallbackInfo<Value> &args) {
    ...
    jobject jreceiver = getResult(env, context, v8, md->v8RuntimePtr, receiver,
                                  top_deeke_jni_v8_V8_UNKNOWN);
    jobject resultObject = env->CallObjectMethod(v8, v8CallObjectJavaMethodMethodID, md->methodID,
                                                 jreceiver, parameters);
    if (env->ExceptionCheck()) {
        resultObject = nullptr;
        Isolate *isolate = getIsolate(env, md->v8RuntimePtr);
        reinterpret_cast<V8Runtime *>(md->v8RuntimePtr)->pendingException = env->ExceptionOccurred();
        env->ExceptionClear();
        jstring exceptionMessage = (jstring) env->CallObjectMethod(
                reinterpret_cast<V8Runtime *>(md->v8RuntimePtr)->pendingException,
                throwableGetMessageMethodID);
        if (exceptionMessage != nullptr) {
            Local<String> v8String = createV8String(env, isolate, exceptionMessage);
        ...
        if (returnType == top_deeke_jni_v8_V8_INTEGER) {
            args.GetReturnValue().Set(getInteger(env, resultObject));
        } else if (returnType == top_deeke_jni_v8_V8_BOOLEAN) {
            args.GetReturnValue().Set(getBoolean(env, resultObject));
        } else if (returnType == top_deeke_jni_v8_V8_DOUBLE) {
            args.GetReturnValue().Set(getDouble(env, resultObject));
        } else if (returnType == top_deeke_jni_v8_V8_STRING) {
            jstring stringResult = (jstring) resultObject;
            Local<String> result = createV8String(env,
                                                  reinterpret_cast<V8Runtime *>(md->v8RuntimePtr)->isolate,
                                                  stringResult);
            args.GetReturnValue().Set(result);
        } else if (returnType == top_deeke_jni_v8_V8_V8_ARRAY) {
            if (isUndefined(env, resultObject)) {
                args.GetReturnValue().SetUndefined();
            } else {
                jlong resultHandle = getHandle(env, resultObject);
                Handle<Object> result = Local<Object>::New(isolate,
                                                           *reinterpret_cast<Persistent<Object> *>(resultHandle));
                releaseArray(env, resultObject);
                args.GetReturnValue().Set(result);
            }
        } else if (returnType == top_deeke_jni_v8_V8_V8_OBJECT) {
            if (isUndefined(env, resultObject)) {
                args.GetReturnValue().SetUndefined();
            } else {
                jlong resultHandle = getHandle(env, resultObject);
        ...
        } else if (returnType == top_deeke_jni_v8_V8_V8_ARRAY_BUFFER) {
            if (isUndefined(env, resultObject)) {
                args.GetReturnValue().SetUndefined();
            } else {
                jlong resultHandle = getHandle(env, resultObject);
...
JNIEXPORT jlongArray JNICALL Java_top_deeke_jni_v8_V8__1initNewV8Function
        (JNIEnv *env, jobject, jlong v8RuntimePtr) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1setWeak
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,);
    WeakReferenceDescriptor *wrd = new WeakReferenceDescriptor();
    wrd->v8RuntimePtr = v8RuntimePtr;
    wrd->objectHandle = objectHandle;
    reinterpret_cast<Persistent<Object> *>(objectHandle)->SetWeak(wrd,
                                                                  [](v8::WeakCallbackInfo<WeakReferenceDescriptor> const &data) {
                                                                      WeakReferenceDescriptor *wrd = data.GetParameter();
                                                                      JNIEnv *env;
                                                                      getJNIEnv(env);
                                                                      jobject v8 = reinterpret_cast<V8Runtime *>(wrd->v8RuntimePtr)->v8;
                                                                      env->CallVoidMethod(v8,
                                                                                          v8WeakReferenceReleased,
                                                                                          wrd->objectHandle);
                                                                      delete (wrd);
                                                                  }, WeakCallbackType::kParameter);
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1clearWeak
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,)
    reinterpret_cast<Persistent<Object> *>(objectHandle)->ClearWeak();
}

JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1isWeak
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
    return reinterpret_cast<Persistent<Object> *>(objectHandle)->IsWeak();
}

JNIEXPORT jlong JNICALL Java_top_deeke_jni_v8_V8__1registerJavaMethod
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jstring functionName,
         jboolean voidMethod) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0)
...
JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1releaseMethodDescriptor
        (JNIEnv *, jobject, jlong, jlong methodDescriptorPtr) {
    MethodDescriptor *md = reinterpret_cast<MethodDescriptor *>(methodDescriptorPtr);
    delete (md);
}

JNIEXPORT void JNICALL Java_top_deeke_jni_v8_V8__1setPrototype
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jlong prototypeHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr,)
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(objectHandle));
    Handle<Object> prototype = Local<Object>::New(isolate,
                                                  *reinterpret_cast<Persistent<Object> *>(prototypeHandle));
    Maybe<bool> maybe = object->SetPrototype(context, prototype);
}

JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1equals
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jlong thatHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(objectHandle));
    Handle<Object> that = Local<Object>::New(isolate,
                                             *reinterpret_cast<Persistent<Object> *>(objectHandle));
    if (objectHandle == 0) {
...
JNIEXPORT jstring JNICALL Java_top_deeke_jni_v8_V8__1toString
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, 0);
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(objectHandle));
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1strictEquals
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jlong thatHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(objectHandle));
    Handle<Object> that = Local<Object>::New(isolate,
                                             *reinterpret_cast<Persistent<Object> *>(thatHandle));
...
JNIEXPORT jboolean JNICALL Java_top_deeke_jni_v8_V8__1sameValue
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle, jlong thatHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(objectHandle));
...
JNIEXPORT jint JNICALL Java_top_deeke_jni_v8_V8__1identityHash
        (JNIEnv *env, jobject, jlong v8RuntimePtr, jlong objectHandle) {
    Isolate *isolate = SETUP(env, v8RuntimePtr, false);
    Handle<Object> object = Local<Object>::New(isolate,
                                               *reinterpret_cast<Persistent<Object> *>(objectHandle));
...
void throwParseException(JNIEnv *env, const char *fileName, int lineNumber, String::Value *message,
    ...
    jthrowable result = (jthrowable) env->NewObject(v8ScriptCompilationCls,
                                                    v8ScriptCompilationInitMethodID, jfileName,
                                                    lineNumber, jmessage, jsourceLine, startColumn,
                                                    endColumn);
...
void
throwExecutionException(JNIEnv *env, const char *fileName, int lineNumber, String::Value *message,
                        String::Value *sourceLine, int startColumn, int endColumn,
                        const char *stackTrace, jlong v8RuntimePtr) {
    jstring jfileName = env->NewStringUTF(fileName);
    jstring jmessage = env->NewString(**message, message->length());
    jstring jsourceLine = env->NewString(**sourceLine, sourceLine->length());
    ...
    jthrowable result = static_cast<jthrowable>(env->NewObject(v8ScriptExecutionException,
                                                               v8ScriptExecutionExceptionInitMethodID,
                                                               jfileName, lineNumber, jmessage,
                                                               jsourceLine, startColumn, endColumn,
                                                               jstackTrace, wrappedException));
    env->DeleteLocalRef(jfileName);
    env->DeleteLocalRef(jmessage);
    env->DeleteLocalRef(jsourceLine);
    (env)->Throw(result);
...
void throwExecutionException(JNIEnv *env, const Local<Context> &context, Isolate *isolate,
    ...
    if (message.IsEmpty()) {
        ...
        if (!v8StackTrace.IsEmpty()) {
            ...
            throwExecutionException(env, filenameString, lineNumber, &exception, &sourceline, start,
                                    end, stackTrace, v8RuntimePtr);
        } else {
            throwExecutionException(env, filenameString, lineNumber, &exception, &sourceline, start,
                                    end, nullptr, v8RuntimePtr);
        }
    }
}
...
void throwV8RuntimeException(JNIEnv *env, String::Value *message) {
    ...
    jthrowable exception = (jthrowable) env->NewObject(v8RuntimeExceptionCls,
                                                       v8RuntimeExceptionInitMethodID,
                                                       exceptionString);
    (env)->Throw(exception);
    env->DeleteLocalRef(exceptionString);
...
jobject getResult(JNIEnv *env, const Local<Context> &context, jobject &v8, jlong v8RuntimePtr,
    ...
    if (result->IsUndefined() && expectedType == top_deeke_jni_v8_V8_V8_ARRAY) {
        jobject objectResult = env->NewObject(undefinedV8ArrayCls, undefinedV8ArrayInitMethodID,
                                              v8);
        return objectResult;
    } else if (result->IsUndefined() && (expectedType == top_deeke_jni_v8_V8_V8_OBJECT ||
                                         expectedType == top_deeke_jni_v8_V8_NULL)) {
        jobject objectResult = env->NewObject(undefinedV8ObjectCls, undefinedV8ObjectInitMethodID,
                                              v8);
        return objectResult;
    } else if (result->IsInt32()) {
        return env->NewObject(integerCls, integerInitMethodID,
                              result->Int32Value(context).FromJust());
    } else if (result->IsNumber()) {
        return env->NewObject(doubleCls, doubleInitMethodID,
                              result->NumberValue(context).FromJust());
        ...
        reinterpret_cast<Persistent<Object> *>(resultHandle)->Reset(isolate, result->ToObject(
                context).ToLocalChecked());
        ...
        if (arrayBuffer->ByteLength() == 0 || arrayBuffer->GetBackingStore()->Data() == nullptr) {
            jobject objectResult = env->NewObject(v8ArrayBufferCls, v8ArrayBufferInitMethodID, v8,
                                                  NULL);
            jlong resultHandle = getHandle(env, objectResult);
            v8::Isolate *isolate = reinterpret_cast<V8Runtime *>(v8RuntimePtr)->isolate;
            reinterpret_cast<Persistent<Object> *>(resultHandle)->Reset(isolate, result->ToObject(
        ...
        jobject byteBuffer = env->NewDirectByteBuffer(arrayBuffer->GetBackingStore()->Data(),
                                                      static_cast<jlong>(arrayBuffer->ByteLength()));
        jobject objectResult = env->NewObject(v8ArrayBufferCls, v8ArrayBufferInitMethodID, v8,
                                              byteBuffer);
        jlong resultHandle = getHandle(env, objectResult);
